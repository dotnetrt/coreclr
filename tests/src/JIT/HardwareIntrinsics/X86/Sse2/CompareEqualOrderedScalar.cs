...
namespace IntelHardwareIntrinsicTest
{
    internal static partial class Program
    {
        ...
        static unsafe int Main(string[] args)
        {
            ...
            if (Sse2.IsSupported)
            {
                using (var doubleTable = TestTableImmSse2<double, double, bool>.Create(testsCount, 2.0))
                {
                    for (int i = 0; i < testsCount; i++)
                    {
                        (Vector128<double>, Vector128<double>, bool) value = doubleTable[i];
                        var result = Sse2.CompareEqualOrderedScalar(value.Item1, value.Item2);
                        doubleTable.SetOutArray(result, value.Item1);
                    }
