            ...
            if (Sse2.IsSupported)
            {
                using (var doubleTable = TestTableSse2<double>.Create(testsCount))
                {
                    for (int i = 0; i < testsCount; i++)
                    {
                        ...
                        var result = Sse2.AddScalar(value.Item1, value.Item2);
                    ...
                    {
                        PrintError(doubleTable, methodUnderTestName, "(double x, double y, double z, ref double a) => (a = (double)(x & y)) == z", checkDouble);
                        testResult = Fail;
                    }
                }
            }
